...
a5.dp.info = {
  engine: `${a5.Engine.version || 'development'} (recommended: buckminster-33.0.3)`,
  root: 'core-release33-2.3.2'
};
...
/*
  ENGINE FILL-IN
  Adjusts layout dimensisons here and there
*/
$(function() {
  a5.callbacks.interaction.bind("loaded", function causeReflow(interaction) {
    var $footer = $('#footer-wrap');

    $footer.css({ opacity: 0.999 }); // apply for a tick to cause reflow
    _.defer(function() { $footer.css({ opacity: '' }); }); // neutralize it in the next tick
  });
});
...
(function($) {
  ...
  var keyboard = a5.dp.keyboard = {
    ...
    behavior: {
      ...
      '.ic-number-layers':               { tabindex: '.input-text input' },
      '.ic-number-pyramid':              { tabindex: '.input-text input' },
      '.ic-number-triangle':             { tabindex: '.input-text input' },
      '.ic-number-wall':                 { tabindex: '.input-text input' },
      '.ic-text-gap':                    { tabindex: '.input-text input' },
      '.ic-text-gap-label-an-object':    { tabindex: '.tehs_view input' },
...
$(function() {
  // This is needed because the supplements area is downscaled using "tranform: scale(value)" rules.
  // But for layouting, the original (unscaled) size is used. So the container of the supplements area
  // still gets the full height
  function adjustMediaContainerHeight(interaction) {
    if (!a5.dp.config.mediaAreaScaleFixWrap || a5.dp.config.responsiveSupplements) { return; }

    var $mediaZone = interaction.$(".is-media-zone");
    $mediaZone.each(function() {
      var $container = $(this).parent();
      _.defer(function(that) {
        // has to be deferred so it runs after the engine resizes the media zone to fit content
        // see engine's a5.Interaction.setMediaBounds
        if (!$container.hasClass("is-media-wrapper")) {
          $(that).wrap("<div class='is-media-wrapper'>");
          $container = $(that).parent();
        }
        $container.height(that.getBoundingClientRect().height);
      }, this);
    });

    if (adjustMediaContainerHeightResizeHooked) { return; }
    adjustMediaContainerHeightResizeHooked = _.throttle(adjustMediaContainerHeight, 300);
    $(window).resize(function() {
      adjustMediaContainerHeightResizeHooked(a5.mainBuilder.curInteraction);
    });
  }
  var adjustMediaContainerHeightResizeHooked;

  a5.hooks.interactionShowed.add(adjustMediaContainerHeight);

  // Corrects jQueryUI Tabs elements with scale transformation compensation
  // relative to the width in the layout template inside which media are arranged by the writer
  function adjustMediaTabs(interaction) {
    var $mediaZone = interaction.$('.is-media-zone.ui-tabs:not(.stay-in-view)');
    $mediaZone.each(function() {
      var transform = $(this).find('.tabnav').css('transform').match(/matrix\(([^,]*),\s*0,\s*0,\s*[^,]*,\s*0,\s*0\)/),
          scale = 1;
      if (transform) {
        scale = transform[1];
      }
      $(this).find('.tab,.tabnav').outerWidth($(this).outerWidth() / scale);
    });

    if (adjustMediaTabsResizeHooked) { return; }
    adjustMediaTabsResizeHooked = _.throttle(adjustMediaTabs, 300);
    $(window).resize(function() {
      adjustMediaTabsResizeHooked(a5.mainBuilder.curInteraction);
    });
  }
  var adjustMediaTabsResizeHooked;

  a5.hooks.interactionShowed.add(adjustMediaTabs);
});

/*
  Manages the audio player speed menu popup toggling.
*/
(function() {
  a5.callbacks.interaction.bind('loaded', function(interaction) {
  interaction.$('.speed-menu,.mediaPlayer__toggleableMenu--quality').on('click', function(e) {
      var $popup = $(this).find('.speed-popup,.mediaPlayer__qualityList');
      if ($(e.currentTarget).is('.speed-menu')) {
        $popup.toggle();
      }
      $popup.removeClass('to-bottom');
      $popup.toggleClass('to-bottom', $popup.offset().top < 0);
    });
  });
})();
...
_.extend(a5.dp.info, {
  child2: 'richmond-rdp-secondary-3.4.1'
});
